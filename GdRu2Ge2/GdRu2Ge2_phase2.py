...
Q5=np.array([q_len,q_len,0.0])
Q6=np.array([q_len,-q_len,0.0])

FH = open("GdRu2Ge2_phase2.vtk","w")   #ファイル名にはスペースを使わないようにしましょう。
...
for ii in range(points_x):
    for jj in range(points_y):
        ...
        phase5=(2.0*np.pi)*np.dot(r_vec2,Q5)
        phase6=(2.0*np.pi)*np.dot(r_vec2,Q6)
        ...
        SA5_vec=np.array([0,0,-1])
        SB5_vec=np.array([-1,1,0])/math.sqrt(2)
        SA6_vec=np.array([0,0,-1])
        SB6_vec=np.array([1,1,0])/math.sqrt(2)


        Mz=1.5#2가 되면 스킬미온이 아니게됨. 스핀적분이 4pi가 되도록하려면..? 계산할 수도 있지만, 1(?)~1.9정도쯤 중앙부가 나오고 메론-안티메론상태가 없어지도록 해야함. 0일땐 제로자기장이라서 스킬미온이 아닌 메론-안티메론 상태가 됨.(IC-1)
        ...
        S4=np.cos(phase4)*SA4_vec+np.sin(phase4)*SB4_vec
        S5=np.cos(phase5)*SA5_vec+np.sin(phase5)*SB5_vec
        S6=np.cos(phase6)*SA6_vec+np.sin(phase6)*SB6_vec

        S_all=S1+S2+0.1*(S3+S4)+(S5+S6)+Sz_vec
